...
### 2.4. セキュアコーディング境界
AIがコードを生成する際は、以下の頻出脆弱性を未然に防ぐため、これらのルールを**絶対的な制約**として遵守してください。

- **外部入力の検証**:
  - 全ての外部入力（HTTPリクエスト、ファイル、DBからの読み込み等）は信頼できないものとして扱ってください。
  - `pydantic`モデルでの型検証や、`shared.security`ユーティリティを使用したバリデーションとサニタイズを**必ず**実施してください。

- **OSコマンドインジェクション対策**:
  - `subprocess`モジュールを使用する際は、`shell=True`を**原則禁止**とします。
  - コマンド引数はリスト形式で渡し、外部からの入力を直接コマンド文字列に結合しないでください。
  - 例: `subprocess.run(["ls", "-l", user_input], check=True)`

- **安全でないデシリアライゼーションの禁止**:
  - `pickle`, `PyYAML`の`yaml.load`など、信頼できないデータをデシリアライズする可能性のある危険な関数は**使用禁止**です。
  - 代わりに`json`モジュールや`yaml.safe_load`を使用してください。

- **SQLインジェクション対策**:
  - 生のSQLクエリを文字列結合（f-stringや`+`演算子）で組み立てることを**固く禁止**します。
  - ORM（Object-Relational Mapper）を利用するか、データベースライブラリが提供するパラメータ化クエリ（プレースホルダ、例: `%s`）を**必ず**使用してください。

- **パス・トラバーサル対策**:
  - 外部入力からファイルパスを構築する場合、`shared.security.is_safe_filename`関数を使用するか、厳格なチェックを行い、ディレクトリ構造を遡る攻撃（例: `../../secret.txt`）を防止してください。

- **一時ファイルの安全な扱い**:
  - 一時ファイルやディレクトリの作成には、予測可能または固定のパス（例: `/tmp/myappfile`）を使用せず、必ず`tempfile`モジュール（例: `tempfile.NamedTemporaryFile`, `tempfile.TemporaryDirectory`）を使用してください。
